...
class TimesFm:
  ...
  def load_from_checkpoint(
    ...
    if (checkpoint_path is None) or use_orbax:
      orbax_path = checkpoint_path
    ...
    if use_orbax:
      placeholder_state = flax_train_state.TrainState.create(
          apply_fn=self._model.apply,
          params=self._train_state.mdl_vars,
          tx=optax.sgd(0)
      )
      new_train_state = flax_checkpoints.restore_checkpoint(orbax_path, placeholder_state)
      self._train_state = self._train_state.new_state(mdl_vars=new_train_state.params, opt_states=[])
    self._logging(
